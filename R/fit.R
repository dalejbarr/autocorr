...
#'
#' Simulate 2x2 data and fit GAMM models.
#'
#' @param n_subj Number of subjects.
#' @param n_obs Number of observations per subject.
#' @param fixed vector of fixed effects in the following order:
...
#' @importFrom mgcv s
#' @export
gamm_2x2 <- function(n_subj, n_obs, fixed, amx, amx_wt = NULL) {
  dat <- gen_2x2(n_subj, n_obs, fixed, amx, amx_wt) %>%
...
#' @importFrom magrittr "%>%"
#' @export
gen_2x2 <- function(n_subj, n_obs, fixed, amx, amx_wt = NULL) {
  ...
  sim_2x2(n_subj, n_obs, parms, amx, amx_wt) %>%
